...
#| @contact  : android-dnadroid@amossys.fr                                     |
...
from dnadroid.analysis.Analysis import Analysis
from dnadroid.analysis.AutomaticAnalysisConfiguration import AutomaticAnalysisConfiguration
from dnadroid.report.Reporter import Reporter
from dnadroid.report.ReportingConfiguration import ReportingConfiguration
from dnadroid.analysis.MainConfiguration import MainConfiguration
from dnadroid.analysis.StaticAnalysis import StaticAnalysis
from dnadroid.device.TelnetEmulation import TelnetEmulation
	...
	for f in onlyfiles:
		...
		with open(path+f) as data_file:
			...
			for line in content:
				...
				results[idx]={'package': d['PackageName'], 'class': d['ClassName'], 'method': d['MethodName'], 'timestamp': d['Timestamp'],'inputs': d['Parameters'],'outputs': d['Return']}
				idx = idx + 1
        ...
        try:
            ...
            for order in analysisConfiguration.scenario:
                if "execute" == order:
                    ...
                    logger.info("Accepting Installation")
                    Analysis.reportEvent(reporter, idXp, emulatorName, "Accepting Installation")
                    device.accept()
            ...
            while True:
                ...
                for directory in self.analysisConfiguration.apkFiles:
                    ...
                    if len(filenames) == 0:
                        ...
                        try:
                            if r.get(5):
				self._logger.info(filenames)
				self._logger.info(str(r.get(5)))
                                self._logger.info("Threads have returned, quitting2")
                                break
                        except TimeoutError:
                            continue
                    else:
    ...
    def __prepareAnalysis(self, commandLineParser):
        ...
        for order in analysisOptions['scenario'].split(","):
            if order is not None and len(order)>0:
                if 'execute' == order or 'stimulate' == order or 'reboot' == order or 'externalStimulation' == order or 'accept' == order:
                    #if 'stimulate' == order:
